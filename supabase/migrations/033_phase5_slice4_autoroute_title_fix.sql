-- Migration 033: Hotfix for migration 032's RPC.
-- cc_issues has NOT NULL constraint on `title` and `summary` may also be
-- referenced — the auto-route claim RPC must populate the top-level title
-- column, not just nest it in `detail`. Drop and recreate the function.
...
BEGIN
  ...
  -- Existing-send idempotency (Layer 1)
  ...
  -- Per-decision cc_issues row (populate title column directly per schema)
  ...
  -- Idempotency Layer 2 (atomic insert; race-safe via partial unique index)
  BEGIN
